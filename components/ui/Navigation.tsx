...
export function Navigation() {
  ...
  return (
    <nav
      ...
      className="pointer-events-none fixed inset-x-0 bottom-6 z-20 flex flex-col items-center gap-4 px-4 font-mono select-none"
    >
      {/* Realm Navigator Bar */}
      <div
        role="tablist"
        aria-label="Cosmological Realms"
        className="pointer-events-auto flex items-center gap-1.5 p-1.5 rounded-full bg-void-950/80 border border-void-700/60 backdrop-blur-xl shadow-2xl"
      >
        {REALMS.map((r) => {
          ...
          return (
            <button
              ...
              className={`relative px-4 py-2 rounded-full text-xs font-medium tracking-wider transition-all flex items-center gap-2 focus-visible:ring-2 focus-visible:ring-amber-glow focus-visible:outline-none ${
                isTarget
            ...
            >
              <span className={`text-[9px] ${isActive ? "text-amber-glow" : "text-void-500"}`}>
                {r.number}
              </span>
              <span>{r.label}</span>
              {!r.available && (
                <span className="text-[8px] border border-void-800 px-1 rounded text-void-500">
                  PHASE 2+
                </span>
              )}
            </button>
      ...
      <div
        ...
        className="pointer-events-auto flex items-center gap-1 text-[10px] text-void-400 bg-void-950/70 border border-void-800/80 px-2.5 py-1 rounded-full backdrop-blur-md"
      >
        <span className="text-void-600 mr-1.5 uppercase tracking-wider">CAM:</span>
        <button
          onClick={() => handleCameraMode("cinematic")}
          aria-pressed={cameraMode === "cinematic"}
          aria-label="Switch to Cinematic Camera"
          className={`px-2 py-0.5 rounded flex items-center gap-1 transition-colors focus-visible:ring-1 focus-visible:ring-ion-cyan focus-visible:outline-none ${
