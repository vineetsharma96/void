/**
 * Fast 3D Simplex-like & Perlin Gradient Noise for procedural generation
 * Implemented with zero external dependencies.
 */

// Permutation table initialized deterministically
...
export function perlin3D(x: number, y: number, z: number): number {
  ...
  return lerp(
    ...
    lerp(
      vFade,
      lerp(uFade, grad(PERM[AA], u, v, w), grad(PERM[BA], u - 1, v, w)),
      lerp(uFade, grad(PERM[AB], u, v - 1, w), grad(PERM[BB], u - 1, v - 1, w))
    ),
    lerp(
      vFade,
      lerp(uFade, grad(PERM[AA + 1], u, v, w - 1), grad(PERM[BA + 1], u - 1, v, w - 1)),
      lerp(uFade, grad(PERM[AB + 1], u, v - 1, w - 1), grad(PERM[BB + 1], u - 1, v - 1, w - 1))
    )
  );
}
